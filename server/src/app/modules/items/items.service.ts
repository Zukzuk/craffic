...
import { CreateItemDto, PatchItemDto, UpdateItemDto } from './dtos/item.dto';
...
export class ItemsService {
  ...
  async create(itemData: CreateItemDto) {
    const item = { id: ++this.idIndex, ...itemData } as IItem;
  ...
  async update(id: number, itemData: UpdateItemDto) {
    let updatedItem: IItem;
    const index = await this.items.findIndex((item) => item.id === id);
    if (index >= 0) {
      const item = this.items[index];
      updatedItem = { id, ...item, ...itemData } as IItem;
  ...
  async patch(id: number, itemData: PatchItemDto) {
